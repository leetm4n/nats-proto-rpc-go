...
func (g *generator) addServiceServerRunnableImpl(service *protogen.Service) {
	...
	for _, method := range service.Methods {
		...
		g.gen.P(fmt.Sprintf("        ctx, span := s.tracer.Start(ctx, \"%s\", trace.WithSpanKind(trace.SpanKindServer))", g.getMethodSubject(method)))
		g.gen.P("        defer span.End()")
		g.gen.P("        ctxWithSpan := s.propagator.Extract(ctx, telemetry.NatsHeaderCarrierFromNatsHeader(request.Headers()))")
		g.gen.P("        if span.IsRecording() {")
		g.gen.P("          span.SetAttributes(")
		g.gen.P(fmt.Sprintf("            attribute.String(\"nats.serviceVersion\", \"%s\"),", g.getServiceVersion(service)))
		g.gen.P(fmt.Sprintf("            attribute.String(\"nats.service\", \"%s\"),", g.getServiceSubject(service)))
